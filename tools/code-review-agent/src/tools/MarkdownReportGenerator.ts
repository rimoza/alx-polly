...
import { ReviewSummary, ReviewComment, CommitMessageConfig, AnalysisResult } from '../types/index.js';

export class MarkdownReportGenerator {
  constructor(private outputPath: string = './code-review-report.md') {}
  ...
  private buildMarkdownReport(result: AnalysisResult): string {
...
`;
  ...
  private buildExecutiveSummary(summary: ReviewSummary): string {
...
`;
  ...
  private buildMetricsSection(summary: ReviewSummary): string {
...
`;
  ...
  private buildFindingsSection(comments: ReviewComment[]): string {
...
`;
  ...
  private buildCommitMessageSection(commitConfig: CommitMessageConfig): string {
...
`;
  ...
  private buildRecommendationsSection(summary: ReviewSummary): string {
...
`;
  ...
  private buildQuickSummary(summary: ReviewSummary): string {
...
`;
  ...
  private groupCommentsByFile(comments: ReviewComment[]): { [file: string]: ReviewComment[] } {
  ...
  private getCategoryBadge(category: string): string {
    ...
    return badges[category as keyof typeof badges] || '![General](https://img.shields.io/badge/Category-General-gray)';
  }

  private getStatusIcon(value: number, warning: number, good: number): string {
    if (value <= good) return '🟢 Good';
    if (value <= warning) return '🟡 Moderate';
    return '🔴 High';
  }

  private getComplexityStatus(complexity: number, linesOfCode: number): string {
  ...
  private generateQualityChart(score: number): string {
...
\`\`\``;
  ...
  private generateRecommendations(summary: ReviewSummary) {
